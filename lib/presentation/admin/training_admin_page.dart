...
class _TrainingAdminPageState extends State<TrainingAdminPage> {
  ...
  Future<void> _publishTraining(dynamic training) async {
    ...
    try {
      setState(() => _loading = true);
      await _supabase.from('thix_trainings').update({'is_published': true}).eq('id', trainingId);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('✅ "$trainingTitle" publiée !'), backgroundColor: Colors.green));
      _loadTrainings();
    } catch (e) {
  ...
  Future<void> _unpublishTraining(dynamic training) async {
    ...
    try {
      setState(() => _loading = true);
      await _supabase.from('thix_trainings').update({'is_published': false}).eq('id', trainingId);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('⏸️ "$trainingTitle" dépubliée'), backgroundColor: Colors.orange));
      _loadTrainings();
    } catch (e) {
  ...
  Future<void> _deleteTraining(dynamic training) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Êtes-vous sûr de vouloir supprimer "$trainingTitle" ?'),
        actions: [
    ...
    try {
      setState(() => _loading = true);
      await _supabase.from('thix_trainings').delete().eq('id', trainingId);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ Formation supprimée'), backgroundColor: Colors.red));
      _loadTrainings();
    } catch (e) {
